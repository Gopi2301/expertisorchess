...
export const ClientCalendar: React.FC = () => {
  ...
  const fetchClasses = useCallback(async () => {
    setLoading(true);
    try {
      const res = await classesApi.listForClient({
      ...
      addToast('Failed to load your family schedule', 'error');
    } finally {
      setLoading(false);
    }
  }, [dateRange]);
  ...
  const infoBanner = pendingCount > 0 ? (
    <div className="flex items-start gap-2 text-sm text-text-muted">
      ...
      <span>
        ...
        {' '}shown as faded — enrollment is pending confirmation.
      </span>
    </div>
  ) : null;
  ...
  const getStudentNames = (cls: Class) => {
     return cls.students?.map(s => s.student?.name).filter(Boolean).join(', ') || 'Unknown Student';
  };
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
            <CalendarDays size={22} className="text-bg-brand" /> Family Schedule
          </h1>
              ...
              {upcomingConfirmed.map(cls => (
                <div key={cls.id}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-medium text-text-secondary mt-0.5">Student(s): {getStudentNames(cls)}</p>
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-text-muted mt-0.5">{formatDateTime(cls.scheduled_start)}</p>
                        <p className="text-xs text-text-secondary mt-0.5">Student: {getStudentNames(cls)}</p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <ClassStatusBadge status={cls.status} />
